 ...
 */
const std::string VERSION = "0.4.25022853";
...
bool getInput(int* val) {
    ...
    std::string input;
    if(!(std::cin>>input)){//xử lí nếu stream bị lỗi
        return false;
    }
    if(!validateInput(input)){//trả về false nếu invalid input
        return false;
    }
    try{
        *val=std::stoi(input);//convert string to int
        return true;
    }
    catch(...){                //bắt lỗi overflow do có thể input là 1 string quá lớn(e.g "9999999999999999999")
        return false;
    }
}
...
bool selectBotLevel(BotLevel* levels, const int index) {
    ...
    if (getInput(&choice)){
        switch(choice){
            case 1:
                levels[index]=BotLevel::EASY;//index 0->player1, index 1->player 2
                return true;
            case 2:
...
void showSelectMenu(SelectType selectType) {
    ...
    switch (selectType) {
        ...
        case SelectType::SIZE_UI:
            ...
            std::cout<<"Chon kich thuoc ban co di ban: (3<=size<="<<BOARD_N_MAX<<")\n";
            break;

        case SelectType::GOAL_UI:
            // TODO: display goal selection
            // Example: "Goal Input (3 - 5, goal <= size)"
            std::cout<<"chon so o de thang (3-5, goal<=size)\n";
            break;
...
void showPlayer(int player, bool is_bot) {
    // TODO:
    int display_player=player+1;
    if (is_bot){
        std::cout<<"Dang tinh toan hang trieu nuoc di tiep theo\n";
    }
    else{
        std::cout<<"dau hang diii player "<<display_player<<"\n";
    }
...
void showMove(const int row, const int col) {
    // TODO:
    std::cout<<"o ("<<row<<","<<col<<") da duoc kiem soat";
}
...
void showInvalidMove() {
    // TODO:
    std::cout<<"khong ngo lai gap phai thang lieu, nhap lai di b\n";
}
...
void showResult(const int winner, const bool is_bot) {
    ...
    if (winner==DRAW_RESULT){
        std::cout<<"Bat tay nhau chap nhan ket qua hoa\n";
    }
    else{
        
        if (is_bot){
            std::cout<<"Bao ngay tu dau la thua roi ma\n";
        }
        else{
            std::cout<<"Chieu het trong su ngo ngang cua stockfish\n";
        }
    }
}
/**
    ...
    // initBoard(gameSetup.board, gameSetup.size);
        ...
        if (gameSetup.mode==GameMode::PVE){
            ...
            while (!selectBotLevel(gameSetup.levels,1)){
                if (config.interactive)std::cout<<"chon lai bot di b\n";
                if (config.interactive)showSelectMenu(SelectType::BOT_LEVEL_UI);
            }
        }
        else if (gameSetup.mode==GameMode::EVE){
            if (config.interactive)std::cout<<"setup bot(X) \n";
            while (!selectBotLevel(gameSetup.levels,0)){
                if (config.interactive) std::cout<<"Chon lai level cho bot 1 di b\n";
            }
            if (config.interactive)std::cout<<"setup bot(O)\n";
            while (!selectBotLevel(gameSetup.levels,1)){
                if (config.interactive)std::cout<<"Chon lai level cho bot 2 di b\n";
            }
        }
    
    initBoard(gameSetup.board,gameSetup.size);      //in ra bàn cờ
}
...
GameResult playGame(const RunConfig& config,
    ...
    while (!isGameOver){
        if (config.interactive){            //nếu đang ở chế độ tương tác->cập nhật giao diện mới
            clearScreen();
            displayBoard(gameSetup.board,gameSetup.size); 
        }

        bool isBotTurn=false;
        if (gameSetup.mode==GameMode::EVE){     //nếu game mode có bot->xét trường hơp lượt của bot
            isBotTurn=true;
        }
        ...
        if (config.interactive){                //show player
            showPlayer(currentPlayer,isBotTurn);
        }

        int row, col;                           // khởi tạo tọa độ

        if (isBotTurn){                         // đại khái là nếu là bot thì tính thời gian chạy
            pII movePoint =measureExecutionTime(
                "botMove",
                [&](){
            ...
            row=movePoint.first;
            col=movePoint.second;

            if (config.interactive){            //cho chương trình nghỉ 1 lúc
            ...
            while (true){
                ...
                if (getPlayerMove(&row,&col)){          //nếu nước đi của người trời có input hợp lệ;
                    if(isValidMove(gameSetup.board,gameSetup.size,row,col)){        //nếu hợp lệ trong thế cờ
                        break;
                    }
                    else{                               // ko hợp lệ trong thế cờ
                        if (config.interactive){
                            showInvalidMove();
                        }
                    }
                    }
                    else{                               //định dạng sai, input ko hợp lệ ngay từ ban đầu
                        if (config.interactive){
            ...
            if (config.interactive){                    //hiển thị nước đi 
                showMove(row,col);
            }

            makeMove(gameSetup.board,row,col,symbols[currentPlayer]);       //đánh dấu ô đã đánh&& turn+1;
            turns++;

            if (checkWin(gameSetup.board,gameSetup.size,symbols[currentPlayer],gameSetup.goal)){        //check win draw
                result.winner=currentPlayer;
                result.isBot=isBotTurn;
                result.turns=turns;
                isGameOver=true;
            }

            else if (checkDraw(gameSetup.board,gameSetup.size)){
...
void endGame(const RunConfig& config,
    ...
    if (gameResult.winner==DRAW_RESULT){
        ...
        winnerLog="Ki thu so "+std::to_string(gameResult.winner+1);
    }
    GameLogger::log("Van co ket thuc "+winnerLog+ " thang |Tong so nuoc di: "+std::to_string(gameResult.turns),GameLogger::Level::INFO);
...
bool isEmptyHead(char board[][BOARD_N_MAX],
    ...
    return (board[x][y]==symbol||board[x][y]=='-');
        ...
        for (int j=0;j<size;j++){
            ...
            for (int dir=0;dir<4;dir++){
                ...
                if (count==goal){
                    ...
                    int tail_x=x;
                    int tail_y=y;
                    bool open_head=isEmptyHead(board,size,head_x,head_y,symbol);
                    bool open_tail=isEmptyHead(board,size,tail_x,tail_y,symbol);
                    if (rule==EndRule::NONE){
                        return true;
                    }
                    else if (rule==EndRule::OPEN_ONE){
                        if (open_head||open_tail){
                            return true;
                        }
                    }
...
pII random_pick(char board[][BOARD_N_MAX],
    ...
    // placeholder
    pII empty_cells[BOARD_N_MAX*BOARD_N_MAX];// do đây là mảng tĩnh->kích cỡ phải được truyền vào ở compile time->dùng board_N_Max
    int count=0;
    for (int i=0;i<size;i++){
        for (int j=0;j<size;j++){
            if (board[i][j]=='-'){
                empty_cells[count]={i,j};
                count++;
            }
        }
    }
    if (count==0){                  //nếu ko còn ô nào trống
        return {-1,-1};     
    }
    std::uniform_int_distribution<int> dist(0,count-1);  //lấy số ngẫu nhiên từ 0 đến count-1;
    int rand_idx=dist(generator);                        //liên quan gì đấy đến số 2013
    return empty_cells[rand_idx];
...
pII simple_heuristic(char board[][BOARD_N_MAX],
    ...
    //kiểm tra nếu bot có cơ hội thắng
    for (int i=0;i<size;i++){
        for (int j=0;j<size;j++){
            if (board[i][j]=='-'){
                board[i][j]=botSymbol;
                bool Bot_Can_Win=checkWin(board,size,botSymbol,goal,EndRule::OPEN_TWO);
                board[i][j]='-';
                if (Bot_Can_Win){
                    return {i,j};
                }
    ...
    //kiểm tra nếu người chơi có thể thắng
    for (int i=0;i<size;i++){
        for (int j=0;j<size;j++){
            if (board[i][j]=='-'){
                board[i][j]=playerSymbol;
                bool Player_Can_Win=checkWin(board,size,playerSymbol,goal,EndRule::OPEN_TWO);
                board[i][j]='-';
                if (Player_Can_Win){
                    return {i,j};
                }
...
pII hard_level(char board[][BOARD_N_MAX],
    ...
    int MAX_DEPTH=(size<=4)?6:3; //cho phép bot nhìn trước 6 nước nếu size<=4 và 3 nước nếu size >4

    //Helper function: Quét các ô xung quanh->trả về true nếu có neigh, false nếu ngược lại
    auto hasNeighbors=[&](int r,int c){
    ...
    //Helper function: Đánh giá thế trận (Càng gần trung tâm càng lợi thế)
    auto evalute=[&]()->int{  //Hàm lambda trả về chỉ số đánh giá
        int score=0;
        int center=size/2;
        for (int i=0;i<size;i++){
            for (int j=0;j<size;j++){
                if (board[i][j]==botSymbol){
                    score+=(size-(std::abs(i-center)+std::abs(j-center))); //Bot: Càng gần center->điểm cộng càng lớn
                }
                else if (board[i][j]==playerSymbol){
                    score-=(size-(std::abs(i-center)+std::abs(j-center)));  //Người: Càng gần center->càng nguy hiểm->trừ bớt điểm đi
                }
            }
        }
        return score;
    ...
    std::function<int(int,int,int,bool)> minimax=[&](int depth, int alpha, int beta, bool isMaximizing)->int{
        //alpha: số điểm mà bot có thể nắm chắc dù người chơi đánh như nào chăng nữa
        //beta: số điểm người chơi có thể nắm chắc nếu bot đánh thế nào chăng nữa
        //isMaximizing: true nếu là lượt bot
        if (checkWin(board, size, botSymbol,goal, EndRule::OPEN_TWO)){
            return 10000+depth;     //depth lớn->đi ít nước->win sớm
        }
        if (checkWin(board, size, playerSymbol, goal, EndRule::OPEN_TWO)){
            return -10000-depth;    //depth nhỏ->đi nhiều nước rồi->thua muộn
        }
        if (checkDraw(board, size)){
            return 0;
        }
        if (depth==0){              // nếu depth==0 tức đã đi hết maxDepth-> đánh giá thế trận bàn cờ
            return evalute();
        }

        if (isMaximizing){          //lượt bot: Mục tiêu là max điểm
            int maxEval=-99999;
            bool prune=false;       // cờ hiệu: true->cắt tỉa 
            for (int i=0;i<size&&!prune;i++){
                for (int j=0;j<size&&!prune;j++){
                    if (board[i][j]=='-'&&(size<=4||hasNeighbors(i,j))){    // chỉ đánh thử nếu ô trống và có ô xung quanh(tránh việc thử những trường hợp đánh ở ô tít xa)
                        board[i][j]=botSymbol;                              //đánh thử
                        int eval=minimax(depth-1,alpha,beta, false);        //tính eval cho nước tiếp theo của thế cờ đó
                        board[i][j]='-';                                    //backtracking
                        maxEval=std::max(maxEval,eval);                     //update maxEval tại thế cờ đó
                        alpha=std::max(alpha,eval);                         //update alpha 
                        if (beta<=alpha){       //nếu beta<=alpha(tồn tại thế cờ mà điểm người chơi có thể ép dc-> điểm bot có thể đạt dc-> bất lợi->ko đánh vào)
                            prune=true;
                        }
                    }       
            ...
            for(int i=0;i<size&&!prune;i++){
                for(int j=0;j<size&&!prune;j++){
                    if (board[i][j]=='-'&&(size<=4||hasNeighbors(i,j))){
                        board[i][j]=playerSymbol;
    ...
    for (int i=0;i<size;i++){
        for(int j=0;j<size;j++){
            if (board[i][j]=='-'&&(size<=4||hasNeighbors(i,j))){
                board[i][j]=botSymbol;
                int score=minimax(MAX_DEPTH-1,-99999,99999,false);
                board[i][j]='-';
                if (score>bestScore){
                    bestScore=score;
                    bestMove={i,j};
